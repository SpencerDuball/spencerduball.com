...
import { Seeder, SeedInfo, FileSeedProvider } from "../seed/index.js";
...
export async function seed() {
  ...
  // process the results
  results?.forEach((it) => {
    if (it.status === "Success") spinner.succeed(`Applied seed ${it.seedName} successfully!`);
    else if (it.status === "Error") spinner.fail(`Error applying ${it.seedName}.`);
    else if (it.status === "NotExecuted") spinner.fail(`Seeds not executed due to prior error ${it.seedName}.`);
  });
...
export async function status() {
  ...
  for (let seed of seeds) {
    if (seed.executedAt) spinner.succeed(`Seed ${seed.name} applied.`);
    else spinner.fail(`Seed ${seed.name} not applied.`);
  }
...
export async function undo(name?: string) {
  ...
  let totalAppliedSeeds = 0;
  for (let seed of seeds) seed.executedAt !== undefined && totalAppliedSeeds++;
...
export async function undoAll() {
  ...
  let initialSeed: SeedInfo | null = null;
  for (let seed of seeds) {
    if (seed.executedAt !== undefined) {
      if (!initialSeed) initialSeed = seed;
      else if (seed.executedAt < initialSeed.executedAt!) initialSeed = seed;
    }
  }
