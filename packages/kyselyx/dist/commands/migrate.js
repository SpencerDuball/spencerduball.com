...
export async function migrate() {
    ...
    const { sources: { db }, migrationFolder: fld, } = getConfig();
    ...
    results?.forEach((it) => {
        if (it.status === "Success")
            spinner.succeed(`Applied migration ${it.migrationName} successfully!`);
        else if (it.status === "Error")
            spinner.fail(`Error applying ${it.migrationName}.`);
...
export async function status() {
    ...
    for (let migration of migrations) {
        if (migration.executedAt)
            spinner.succeed(`Migration ${migration.name} applied.`);
        else
            spinner.fail(`Migration ${migration.name} not applied.`);
    }
    // close the database connection
    await db.destroy();
}
...
export async function undo(name) {
    ...
    let totalAppliedMigrations = 0;
    for (let migration of migrations)
        migration.executedAt !== undefined && totalAppliedMigrations++;
...
export async function undoAll() {
    ...
    for (let migration of migrations) {
        if (migration.executedAt !== undefined) {
            ...
            else if (migration.executedAt < initialMigration.executedAt)
                initialMigration = migration;
        }
    }
