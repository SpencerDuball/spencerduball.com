...
//----------------------------------------------------------------------------------------------------------------------
const selectDefaultClasses = "flex items-center justify-between";
const selectConfig = {
  variants: {
    size: {
...
const selectVariants = cva(selectDefaultClasses, selectConfig);
...
interface SelectTriggerProps
  extends Omit<React.ComponentPropsWithoutRef<typeof SelectPrimitive.Trigger>, "disabled">,
    VariantProps<typeof selectVariants> {}

const SelectTrigger = React.forwardRef<React.ElementRef<typeof SelectPrimitive.Trigger>, SelectTriggerProps>(
  ({ size, variant, disabled, invalid, className, children, ...props }, ref) => (
    <SelectPrimitive.Trigger
      ref={ref}
      className={cn(selectVariants({ size, variant, disabled, invalid, className }))}
      disabled={!!disabled}
...
>(({ className, children, position = "popper", ...props }, ref) => (
  <SelectPrimitive.Portal>
    <SelectPrimitive.Content
      ref={ref}
      className={cn(
        "data-[state=open]:animate-in data-[state=closed]:animate-out data-[state=closed]:fade-out-0 data-[state=open]:fade-in-0 data-[state=closed]:zoom-out-95 data-[state=open]:zoom-in-95 data-[side=bottom]:slide-in-from-top-2 data-[side=left]:slide-in-from-right-2 data-[side=right]:slide-in-from-left-2 data-[side=top]:slide-in-from-bottom-2 relative z-50 max-h-96 min-w-[8rem] overflow-hidden rounded-md border bg-slate-1 text-slate-12 shadow-md dark:bg-slatedark-1 dark:text-slatedark-12",
        position === "popper" &&
...
>(({ className, children, ...props }, ref) => (
  <SelectPrimitive.Item
    ref={ref}
    className={cn(
      "relative flex w-full cursor-pointer select-none items-center rounded-sm py-1.5 pl-8 pr-2 text-sm outline-none focus:bg-slate-3 focus:text-slate-12 data-[disabled]:pointer-events-none data-[disabled]:opacity-50 dark:focus:bg-slatedark-3 dark:focus:text-slatedark-12",
      className,
    )}
    {...props}
  >
